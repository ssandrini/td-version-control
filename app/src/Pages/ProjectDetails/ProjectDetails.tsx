...
const ProjectDetail: React.FC = () => {
  ...
  const [changes, setChanges] = useState<ChangeSet<TDNode>>(
    new ChangeSet<TDNode>()
  );
              ...
              <div className="mb-4 text-gray-700">
                ...
                <div className="p-4 rounded">
                  <div className="mb-2">
                    ...
                    <ul className="list-disc ml-5">{/* modified files */}</ul>
                  </div>
                  <div className="mb-2">
                    <h4 className="text-lg font-semibold">New Files</h4>
                    <ul className="list-disc ml-5">{/* new files */}</ul>
                  </div>
                  <div className="mb-2">
                    <h4 className="text-lg font-semibold">Deleted Files</h4>
                    <ul className="list-disc ml-5">{/* deleted files */}</ul>
                  </div>
                </div>
              </div>
        ...
        <div className="bg-gray-800 rounded-lg p-4 flex w-full overflow-auto">
          ...
          <div className="p-4 text-white bg-gray-900">
            ...
            <p>
              ...
              {changes != null && changes.added.length > 0
                ? changes
                    .added
                    .map((node) => [node.name, node.type!].join(": "))
                    .join(", ")
                : "None"}
            </p>
            <p>
              <strong>Deleted Nodes:</strong>{" "}
              {changes != null && changes.deleted.length > 0
                ? changes
                    .deleted
                    .map((node) => node.name)
                    .join(", ")
            ...
            <p>
              ...
              {changes != null && changes.modified.length > 0
                ? changes
                    .modified
                    .map((node) => node.name)
                    .join(", ")
