    ...
    return (<div
        ...
        <div className="flex px-4 flex-row gap-3 py-4 flex-wrap">
            {current?.nodes.map((node, index) => (
                <div key={index} className="h-fit w-28"><OperatorCard node={node}/></div>))}
        </div>
        ...
        <div className="flex flex-row gap-3 px-4 py-4 flex-wrap">
            {changes.added.items.map((change, index) => (<div key={index} className="h-fit w-28"><OperatorCard
                node={change}
                Icon={FaPlus}
                iconColor="text-green-500"
            /></div>))}
            {changes.deleted.items.map((change, index) => (<div key={index} className="h-fit w-28"><OperatorCard
                node={change}
                Icon={FaMinus}
                iconColor="text-red-600"
            /></div>))}
            {changes.modified.items.map((change, index) => (<div key={index} className="h-fit w-28"><OperatorCard
                node={change}
                Icon={FaEdit}
