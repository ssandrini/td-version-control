...
const ProjectDetail: React.FC = () => {
    ...
    const [mergeConflicts, setMergeConflicts] = useState<{currentState: TDState|null, incomingState: TDState|null} | undefined>(undefined);
    ...
    const handlePush = () => {
        // eslint-disable-next-line @typescript-eslint/ban-ts-comment
        // @ts-expect-error
        window.api.push(project?.path).then(() => {
            toast({
                className: "", style: {
                    ...
                    <div className="flex flex-col">
                        ...
                        <div className="font-p1_regular">Your project is now up to date with the cloud.
                        </div>
                    </div>
                </div>)
            })
        ...
        window.api
            ...
            .then((result: TDMergeResult) => {
                if (result.status === TDMergeStatus.FINISHED) {
                    ...
                    setMergeConflicts({currentState: result.currentState, incomingState: result.incomingState});
                }
            }).catch(() => {
                toast({
                    className: "", style: {
    ...
    const handleResolveConflict = (resolvedState: TDState | undefined) => {
        ...
        // eslint-disable-next-line @typescript-eslint/ban-ts-comment
        // @ts-expect-error
        window.api.finishMerge(project.path, resolvedState).then((response) => {
            ...
            <Dialog open>
                <DialogContent className="min-w-[90%] max-w-[90%] w-[90%] min-h-[90%] max-h-[90%] h-[90%] flex flex-col bg-gray-600">
                    ...
                    <div className="flex flex-row gap-5 w-full h-full">
                        <NodeGraph current={mergeConflicts?.currentState ?? undefined} compare={mergeConflicts?.incomingState ?? undefined}/>
                        <NodeGraph current={mergeConflicts?.incomingState ?? undefined} compare={mergeConflicts?.currentState ?? undefined}/>
                    </div>
                </DialogContent>
            </Dialog>
        )}
